package ru.sskie.vpered.food
package controllers

import models.domain.Food
import services.FoodService
import sttp.tapir.generic.auto._
import sttp.tapir.json.circe._
import sttp.tapir.server.ziohttp.{ZioHttpInterpreter, ZioHttpServerOptions}
import sttp.tapir.swagger.bundle.SwaggerInterpreter
import sttp.tapir.ztapir._
import sttp.tapir.PublicEndpoint
import zio._
import zio.http.{Response, Routes}
import zio.metrics.connectors.prometheus.PrometheusPublisher
import sttp.tapir.server.interceptor.metrics.MetricsRequestInterceptor
import sttp.tapir.server.metrics.zio.ZioMetrics
import sttp.tapir.server.model.ValuedEndpointOutput

trait FoodController {
  def routes: Routes[Any, Response]
}

object FoodController {
  val live: URLayer[PrometheusPublisher with FoodService, FoodController] =
    ZLayer.fromFunction(FoodControllerImpl.apply _)
}

final case class FoodControllerImpl(publisher: PrometheusPublisher, foodService: FoodService) extends FoodController {

  private val baseEndpoint: PublicEndpoint[Unit, Unit, Unit, Any] = endpoint.in("api" / "v1")

  // test
  val testEndpoint =
    baseEndpoint
      .tag("Food Service")
      .get
      .in("food")
      .in(path[String]("name"))
      .errorOut(stringBody)
      .out(jsonBody[Food])
      .zServerLogic(name =>
        ZIO
          .succeed(foodService.getStaff(name))
          .flatten
          .mapError(t => t.toString)
      )

  val metricEndpoint =
    baseEndpoint
      .tag("Food Service")
      .get
      .in("metrics")
      .errorOut(stringBody)
      .out(stringBody)
      .zServerLogic(_ => publisher.get)

  // Docs
  val swaggerEndpoints: List[ZServerEndpoint[Any, Any]] =
    SwaggerInterpreter()
      .fromEndpoints[Task](
        List(
          testEndpoint.endpoint,
          metricEndpoint.endpoint
        ),
        "Food Service",
        "1.0"
      )

  def failureResponse(msg: String): ValuedEndpointOutput[_] =
    ValuedEndpointOutput(stringBody, msg)

  val metrics: ZioMetrics[Task]                           = ZioMetrics.default[Task]()
  val metricsInterceptor: MetricsRequestInterceptor[Task] = metrics.metricsInterceptor()
  val serverOptions: ZioHttpServerOptions[Any]            =
    ZioHttpServerOptions.customiseInterceptors
      .metricsInterceptor(metricsInterceptor)
      .defaultHandlers(failureResponse)
      .options

  def routes: Routes[Any, Response] =
    ZioHttpInterpreter(serverOptions).toHttp(
      List(
        testEndpoint,
        metricEndpoint
      ) ++ swaggerEndpoints
    )

}
